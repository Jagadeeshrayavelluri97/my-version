              ...
              {stats.overdueRents.slice(0, 3).map((rent, index) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-semibold">{rent.tenant?.name}</p>
                      <p className="text-sm text-gray-600">
                        Room {rent.room?.roomNumber}, Floor{" "}
                        {rent.room?.floorNumber}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-red-600">₹{rent.amount}</p>
                      <p className="text-xs text-red-600">
                        {rent.daysPastDue} days overdue
                      </p>
                    </div>
                  </div>
                  <div className="mt-2 text-sm">
                    <p>Due on: {new Date(rent.dueDate).toLocaleDateString()}</p>
                    <p>Phone: {rent.tenant?.phone}</p>
                  </div>
                </div>
              ))}
            </div>
